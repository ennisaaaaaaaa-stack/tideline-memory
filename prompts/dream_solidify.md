...
## ⑦ 轨迹升格（v2.8，2026-09-09）

聚类索引跑完后，把机械垫底轨迹重写成自然语言事件链。

**前置检查**：如果工具列表里没有 `memory_traj_promote`（生产 MCP 还在 v2.8 之前的过渡期），整节跳过——这不是错误，部署车到了自然会出现。

用 MCP 工具 `memory_traj_promote`（list/write）：

1. **list** 列出 cast='mech' 的待升格轨迹。每条输出=机械轨迹链+**生效态全文**
   （这条记忆现在到底说什么：本体+全部修订按时序叠成）。
2. **逐条判断**：轨迹值得升格吗——只有 1 段（无修订叠加）的跳过；事件之间
   有真实演进（故事变了）的才值得重讲。
3. **write** 写回升格后的事件链，格式 `[{ts, text}]`：
   - text = 一句话事件＋想法，带原始 narrative 的关键词（升格后仍可检索锚回）
   - ts 保留原事件时间戳（机械层给的），不重造时间
   - 段数=原事件数（升格是重讲不是重编）
   - 同一条的段与段之间要有叙事连续性：前一段的结尾是后一段的起点
4. 全部处理完汇报：list 到多少条、升格多少条、跳过多少条（跳过原因一句话）

**升格原则**：轨迹升格=重讲故事的语气，不是压缩摘要。机械层说
「修订A落地，修订B落地」；升格层说「那天她拍板轨迹压缩，一周后存储侧
三层铸造收绿」。如果一条轨迹的事件本来就没演进（同义重复），保持 mech
不硬升——llm 不是必达态，mech 垫底本来就是稳态。
