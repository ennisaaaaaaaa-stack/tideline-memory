...
If your agent already has conversation history and you want to seed Tideline with it:

**The only required table is `context_records`.** Every row needs:

| Field | Type | Notes |
|-------|------|-------|
| `id` | INTEGER PRIMARY KEY | Auto-increment |
| `session_id` | TEXT | Any stable identifier for the conversation session |
| `role` | TEXT | `"user"`, `"assistant"`, or `"tool"` |
| `content` | TEXT | The raw message text |
| `timestamp` | REAL | Unix timestamp (seconds) |

Everything else (`tags`, `related_entities`, `embedding`) is optional and will be generated by the script layer or DREAM pipeline.

**Steps:**

1. Write your existing messages into `context_records` with the schema above.
2. Run `python3 scripts/dream_scripts.py` to build topic clusters from the imported content.
3. (Optional) Generate embeddings for each row and store in the `embedding` BLOB column. Use `scripts/soft_clusters.py --build` to cluster them.
4. (Optional) Let DREAM solidification run — it will detect the unindexed context records and generate narratives from them automatically.

**From Hermes specifically:** your `sync_turn` table maps directly — `session_id`, `role`, `content`, `timestamp` are the same fields. A simple `INSERT INTO ... SELECT` is enough.

**No import script is provided.** If you're running your own agent, you already know how to move rows between SQLite tables. If not — start fresh, Tideline will learn your agent's history from the next conversation.
